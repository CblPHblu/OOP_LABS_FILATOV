...
TEST(test20, basic_test_set) {
    bool check17 = false;
    Money First_num20("100000,00");
    Money Second_num20("100000,00");
    check17 = First_num20.is_smaller(Second_num20);
    ASSERT_FALSE(check17);
}

TEST(test19, basic_test_set) {
    Money First_num19("100,00");
    Money Second_num19;
    Money Result19(First_num19.remove(Second_num19));
    Money Expected19("100,00");
    ASSERT_TRUE(Result19.is_equals(Expected19));
}

TEST(test22, basic_test_set) {
    Money First_num22("100,00");
    Money Second_num22;
    Money Result22(First_num22.remove(Second_num22));
    Money Expected22("100,00");
    ASSERT_TRUE(Result22.is_equals(Expected22));
}

TEST(test24, basic_test_set) {
    bool check24 = false;
    Money First_num24("100000,00");
    Money Second_num24;
    check24 = First_num24.is_smaller(Second_num24);
    ASSERT_FALSE(check24);
}

TEST(test25, basic_test_set) {
    bool check25 = false;
    Money First_num25("100000,00");
    Money Second_num25;
    check25 = First_num25.is_larger(Second_num25);
    ASSERT_TRUE(check25);
}

TEST(test26, basic_test_set) {
    bool check26 = false;
    Money First_num26;
    Money Second_num26;
    check26 = First_num26.is_equals(Second_num26);
    ASSERT_TRUE(check26);
}
